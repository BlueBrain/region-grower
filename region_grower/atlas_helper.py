...
class AtlasHelper:
    ...
    def lookup_orientation(self, position: Point, vector: Optional[Point] = None) -> np.array:
    ...
    def lookup_target_reference_depths(
        self, position: Point, cortical_depths: List[float]
        ...
        raise RegionGrowerError(
            f"Current depth ({current_depth}) for position ({position}) is"
