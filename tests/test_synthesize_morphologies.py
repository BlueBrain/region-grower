...
from region_grower.placement_algorithm_mpi_app import _run, MASTER_RANK
from atlas_mock import CellCollectionMock, small_O1_placement_algo
...
def check_yaml(ref_path, tested_path):
    ...
    with open(ref_path) as ref_file, open(tested_path) as tested_file:
        ref = yaml.load(ref_file, Loader=yaml.FullLoader)
        tested = yaml.load(tested_file, Loader=yaml.FullLoader)
    assert_equal(ref.keys(), tested.keys())
    for k in ref.keys():
        assert_allclose(ref[k], tested[k])
...
@patch('region_grower.placement_algorithm_utils.CellCollection.load_mvd3',
